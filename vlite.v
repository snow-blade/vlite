...
fn main() {
        ...
        args.parse()
        json_file := os.read_file('conf.json') or {
        ...
        match args.command {
                ...
                'get' {
                        if args.options['p'] in plugins_array {
                                req := http.get('https://raw.githubusercontent.com/rxi/lite-plugins/master/plugins/' +
                                        args.options['p'] + '.lua') or {
                                ...
                                mut fi := os.create(decoded.lite_path + 'data/plugins/' + args.options['p'] + '.lua') or {
                                        println('failed to create ' + decoded.lite_path + 'data/plugins/' +
                                                args.options['p'] + '.lua')
                                        return
                                }
                                fi.write(req.text)
